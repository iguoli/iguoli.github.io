...
- 在几何学中，圆的切线被定义为与圆只有一个公共点（切点）的直线。这是切线最直观的定义，它扩展到其他几何图形，比如椭圆或多边形。
- 在微积分中，一条切线是曲线在某一特定点上的 **直线近似**。更严谨地说，若曲线为函数 $y = f(x)$，那么在点 $(a, f(a))$ 处的切线，设为 $y=L(x)$，是该点的唯一一条直线，其斜率（slope）等于函数在该点的导数 $f'(a)$。这条直线与曲线在 $x=a$ 处通过直线的斜率 $m$ 和曲线的导数 $f'(a)$ 有了交集，的换句话说，这条直线在点 $(a, f(a))$ 附近与函数 $y = f(x)$ 的图形非常接近，斜率 $m$ 为
