...
### 查找 key 为 "foo"，其值不包含 "bar" 的对象

```sh
jq 'select(.foo | contains("bar") | not)' file.json
```

### 查找 key 为 "foo"，值为 "bar"，并且 key 为 "value"，值不包含 "log" 的对象

```sh
jq 'select(.foo == "bar" and (.value|contains("log")|not))' file.json
```

### 查找 key 为 "foo"，其值是 "String" 类型的对象
...
```sh
jq 'select(timestamp == "20221201T12:00:00" and (.foo | contains("bar")))' file.json
```
