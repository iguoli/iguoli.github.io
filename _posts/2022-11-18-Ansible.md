...
参考 [Ansible sudo – ansible become example](https://www.middlewareinventory.com/blog/ansible-sudo-ansible-become-example/) 了解详细说明。

如果希望以非 `root` 用户执行 ansible 任务，`become` 和 `become_user` 必须同时在 playbook 中使用，这相当于执行 `sudo -u someuser` 命令。

如果只使用 `become` 而没有定义 `become_user` 时，相当于执行基本的 `sudo` 命令，即以 `root` 用户执行相应的任务。
...
例如，加密字符串 'foooodev'，其变量名为 'the_dev_secret'，并且使用 *a_password_file* 中的密码 (`abc123`)
...
可以在 `ansible.cfg` 文件中设置 [DEFAULT_VAULT_PASSWORD_FILE](https://docs.ansible.com/ansible/latest/reference_appendices/config.html#default-vault-password-file) 和 [DEFAULT_VAULT_IDENTITY](https://docs.ansible.com/ansible/latest/reference_appendices/config.html#default-vault-identity)
...
vault_password_file=a_password_file
...
- 解密不带 vault id label 格式的字符串，使用 `echo` 命令注意加密字符串中不要有空白
  ...
  36363661386236313833646262666561363461613037646134633830646561383938633261643538
  6439353735643865340a616534613665363836623137333965643062636332353766666166353437
  6535' | ansible-vault decrypt --vault-id a_password_file
  ```

- 解密带 vault id label 格式的字符串，不给出的加密字符串的话，会在命令行提示输入要解密的字符串

  ```sh
  ansible-vault decrypt --vault-id dev@a_password_file
  ```
